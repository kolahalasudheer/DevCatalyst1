  ...
  return (
    <nav className="fixed top-0 w-full bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-white/10 px-4 md:px-8 py-4 z-50">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <button className="md:hidden text-white px-3 py-2 rounded border border-white/10 hover:bg-white/10 transition-colors touch-manipulation" aria-label="Toggle menu" onClick={() => setOpen((v: any)=>!v)}>
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </button>
        <ul className="hidden md:flex gap-8">
          <li><NavLink to="/home" className="text-white hover:text-[#00d4ff] transition-colors relative group">
            Home
